...
#uncomment whichever broken address is appropriate for final integration
#broker="localhost"

mqtt.Client.connected_flag=False# create flag in class
client = mqtt.Client("servnavi")             #create new instance 


camera = cv2.VideoCapture('rtsp://172.20.10.3:8554/mjpeg/1')
    ...
    while True:
        ...
        if largestArea > frameArea*0.02:
            ...
            if (detectedTrafficSign) == "Move Straight":
                client.publish("inTopic9907","0")
            elif (detectedTrafficSign) == "Turn Left":
                client.publish("inTopic9907","1")
            elif (detectedTrafficSign) == "Turn Right":
                client.publish("inTopic9907","2")
            elif (detectedTrafficSign) == "Turn Back":
                client.publish("inTopic9907","3")
            else:
                client.publish("inTopic9907","0")
...
def main():
    ...
    while not client.connected_flag: #wait in loop
        print("Connection is Progress")
        time.sleep(1)
    print("Ready for Publishing inTopic9907")
    findTrafficSign()
